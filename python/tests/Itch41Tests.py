...
class Itch41Tests(TestCase):
    ...
    def test_timestamp_create(self):
        # GIVEN
        message_args = [MessageType.TimeStamp,
                        {Field.Seconds: 100}]

        # WHEN
        message = ItchMessageFactory.create_from_args(message_args)

        # THEN
        assert_that(message.MessageType, equal_to('T'))
        assert_that(message.Seconds, equal_to(100))

    def test_systemevent_create(self):
        # GIVEN
        message_args = [MessageType.SystemEvent, {
        ...
        # THEN
        assert_that(message.MessageType, equal_to('S'))
        assert_that(message.NanoSeconds, equal_to(105))
        assert_that(message.EventCode, equal_to('0') )
